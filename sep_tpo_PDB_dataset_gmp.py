...
class PDBDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if focal_atom is None:
            return self._zero_padded_sample(label, pdb_code)
    ...
    def _zero_padded_sample(self, label, filename):
        """Return an empty tensor instead of a padded one for consistency with global max pooling."""
        return {
            "coordinates": np.zeros((0, 6), dtype=np.float32),  # No fixed max_len
            "label": label,
            "filename": filename,
        }
